...
def get_latest_line(conn, chan, nick, offset):
...
def grab(text, nick, chan, db, conn):
    ...
    otext = text
    split_text = otext.split(' ')
    ntext = split_text[0]

    if ntext.lower() == nick.lower():
    ...
    with grab_lock:
        if len(split_text) == 2 and split_text[1].isnumeric():
            name, timestamp, msg = get_latest_line(conn, chan, ntext, split_text[1])
        else:
            name, timestamp, msg = get_latest_line(conn, chan, ntext, 0)
        ...
        if check_grabs(ntext.casefold(), msg, chan):
...
@hook.command("grabquery", "grabq", autohelp=False)
def grabquery(text, nick, chan, db, conn):
    try:
        results = db.execute(sqlalchemy.sql.text("SELECT COUNT(*) as cnt, name FROM grab WHERE quote LIKE '%' || :quer || '%' GROUP BY name ORDER BY cnt DESC"), {"quer": text})
        grabs = results.mappings().all()
        response = ""
        if len(grabs) == 0:
            response = "No one has been grabbed using " + text + "."
        else:
            response = grabs[0]["name"] + " has been grabbed using " + text + " " + str(grabs[0]["cnt"]) + " time(s)" 
            if len(grabs) > 1:
                response += " followed by " + grabs[1]["name"] + " using it " + str(grabs[1]["cnt"]) + " time(s)"                    
        return response
    except SQLAlchemyError:
        logger.exception(
            "Failed to query"
        )
        return "Failed to lookup grabs."
