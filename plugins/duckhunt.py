...
def attack(event, nick, chan, db, conn, attack_type):
    ...
    if attack_type == "shoot":
        ...
        miss = [
            "SLAM! You missed the duck and hit a wall!",
            "You tried to punch a duck that you were nowhere near, try a gun next time.",
            "You missed. How sad.",
            "You punched a duck but the duck did not care.",
        ]
        no_duck = "There is no duck! What are you trying to punch?"
        msg = "{} you punched a duck in {:.3f} seconds! You have killed {} in {}."
        scripter_msg = (
            "nah"
            "no"
        )
...
@hook.command("punch", autohelp=False)
def punch(nick, chan, db, conn, event):
    """- Punch the duck!"""
    with chan_locks[conn.name][chan.casefold()]:
        return attack(event, nick, chan, db, conn, "punch")
